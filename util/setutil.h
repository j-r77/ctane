...
/*template <typename T>
bool containsSubsetOf(const T& collection, const typename T::key_type& item) {
    for (const typename T::value_type& s : collection) {
        if (isSubsetOf(s.first, item)) return true;
    }
    return false;
}*/
...
/*template <typename T>
T partitionIntersection(const T& lhs, const T& rhs) {
    if (lhs.size() > rhs.size()) return partitionIntersection(rhs, lhs);
    T res;
    res.reserve(lhs.size());

    int startIx = 0;
    for (const typename T::value_type& ltids : lhs) {
        if (ltids.size() == 1) {
            for (const typename T::value_type& rtids : rhs) {
                if (std::binary_search(rtids.begin(), rtids.end(), ltids[0])) {
                    res.push_back(ltids);
                    break;
                }
            }
            ...
            for (int rix = startIx; rix < rhs.size(); rix++) {
                const typename T::value_type& rtids = rhs[rix];
                if (ltids.back() < rtids.front()) break;
                if (rix == startIx && rtids.back() < ltids.front()) startIx++;
                if (rtids.size() == 1) {
                    if (std::binary_search(ltids.begin(), ltids.end(), rtids[0])) {
                        res.push_back(rtids);
                    }
                }
                else {
                    std::vector<int> ijtids = intersection(ltids, rtids);
                    if (ijtids.size()) {
                        res.push_back(std::move(ijtids));
                    }
                }
            }
        }
