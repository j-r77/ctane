...
int BaseMiner::nrCFDs() const {
    return fCFDs.size();
}

const CFDList& BaseMiner::getCFDs() const {
    return fCFDs;
}
...
/*std::vector<MinerNode<PartitionTidList> > BaseMiner::getPartitionSingletons(int minsup, const Database& db) {
    ...
    for (int a = 0; a < db.nrAttrs(); a++) {
        ...
        singletons.back().fTids.resize(dom.size());
        for (int i = 0; i < dom.size(); i++) {
            attrIndices[dom[i]] = std::make_pair(singletons.size() - 1, i);
            singletons.back().fTids[i].reserve(db.frequency(dom[i]));
        }
    }
    ...
    for (unsigned item = 1; item <= db.nrItems(); item++) {
        if (db.frequency(item) >= minsup) {
            ...
            singletons.back().fTids.resize(1);
    ...
    for (unsigned row = 0; row < db.size(); row++) {
        ...
        for (int item : tup) {
            const auto& attrNodeIx = attrIndices.at(item);
            singletons[attrNodeIx.first].fTids[attrNodeIx.second].push_back(row);
            const auto& it = nodeIndices.find(item);
            if (it != nodeIndices.end()) {
                singletons[it->second].fTids[0].push_back(row);
            }
        }
    }
...
SimpleTidList BaseMiner::getConstantVio(const Itemset& lhs, int rhs, const SimpleTidList& tids, const Database& db) {
    SimpleTidList res;
...
SimpleTidList BaseMiner::getVariableVio(const Itemset& lhs, int rhs, const SimpleTidList& tids, const Database& db) {
    SimpleTidList res;
    int rAttr = -1-rhs;
...
bool BaseMiner::precedes(const Itemset& a, const Itemset& b) {
    ...
    for (int i : b) {
        ...
        else if (i >= 0 && std::binary_search(a.begin(), a.end(), -1-fDb.getAttrIndex(i))) {
            c++;
        }
    }
    return c == a.size();
